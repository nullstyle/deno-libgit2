...
The message API provides functions for cleaning up and parsing commit messages,
including prettifying messages and extracting trailers.
...
message. Optionally removes lines starting with the comment character.
...
- `comment_char`: Comment character (lines starting with this are removed if
  strip_comments is non-zero)
...
Parse trailers out of a message. Trailers are key/value pairs in the last
...
2. **Parse trailers**: Extract metadata like "Signed-off-by:",
   "Co-authored-by:", etc.
