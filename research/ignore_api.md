...
- `rules` - Text of rules, a la .gitignore file contents. Multiple rules
...
Clear ignore rules that were explicitly added via `git_ignore_add_rule`. Resets
to default internal ignores (".", "..", ".git"). Does NOT affect rules in actual
.gitignore files.
...
Test if the ignore rules apply to a given path. Equivalent to
