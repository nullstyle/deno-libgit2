...
The pathspec API provides functions for compiling and matching pathspec patterns
against files in a repository. Pathspecs are the patterns used by git commands
to limit operations to specific paths.
...
Try to match a single path against a pathspec. Returns 1 if path matches, 0 if
it does not.
