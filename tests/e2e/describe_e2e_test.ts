  ...
  try {
    ...
    await t.step("describe commit with commits since annotated tag", async () => {
      await using ctx = await createTestContext({ withInitialCommit: true });
        ...
        ctx.repo = Repository.open(ctx.repoPath);
        await createCommitWithFiles(ctx, "Second commit", { "file2.txt": "content2\n" });
        const headOid = ctx.repo.headOid()!;
    ...
    await t.step("describe workdir with dirty suffix", async () => {
      await using ctx = await createTestContext({ withInitialCommit: true });
        ...
        // Make workdir dirty
        await Deno.writeTextFile(`${ctx.repoPath}/file.txt`, "modified content\n");
    ...
    await t.step("describe with TAGS strategy finds lightweight tags", async () => {
      await using ctx = await createTestContext({ withInitialCommit: true });
        ...
        const cmd = new Deno.Command("git", { args: ["tag", "lightweight-tag"], cwd: ctx.repoPath });
        await cmd.output();
