...
Deno.test({
  ...
  fn() {
    ...
    try {
      const repo = createTestRepo();
      createFileAndCommit(repo, "src/lib/file.ts", "export const x = 1;", "Add file");
...
Deno.test({
  ...
  fn() {
    ...
    try {
      ...
      createFileAndCommit(repo, "temp.txt", "Temporary content", "Add temp file");
...
Deno.test({
  ...
  fn() {
    ...
    try {
      ...
      const blockContent = `# Block Title\n\nThis is block content.\n\n[[link-to-other-block]]`;
      ...
      // Create another file
      createFileAndCommit(repo, ".dork/blocks/other-block.md", "Other block", "Add other block");
